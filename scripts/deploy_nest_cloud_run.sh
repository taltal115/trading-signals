...
echo ""
echo "POST /api/github/workflows/* returns 503 until PERSONAL_GITHUB_TOKEN is set on Cloud Run."
echo "  Google Cloud console → Cloud Run → ${SERVICE} → Edit & deploy new revision → Variables"
echo "  or: gcloud run services update ${SERVICE} --region=${REGION} --update-env-vars=PERSONAL_GITHUB_TOKEN=YOUR_PAT"
