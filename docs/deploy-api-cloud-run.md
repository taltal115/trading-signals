...
- **`403 Permission 'run.services.get' denied` on Hosting finalize:** The deploy identity can upload files but cannot read Cloud Run `trading-signals-api`. Grant `roles/run.viewer` to the GitHub deploy SA **and** `service-<PROJECT_NUMBER>@gcp-sa-firebasehosting.iam.gserviceaccount.com`. See [deploy-github-actions.md](./deploy-github-actions.md) (Hosting + Cloud Run rewrite IAM).
